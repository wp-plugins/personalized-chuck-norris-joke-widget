=== Plugin Name ===
Contributors: maarten.decat
Donate link: http://maartendecat.be
Tags: chuck norris, jokes, widget, sidebar, fun
Requires at least: 2.8
Tested up to: 3.0.4
Stable tag: 0.5

Shows a random personalized Chuck Norris joke on your blog, starring yourself. For regular Chuck Norris jokes, please refer to the [Chuck Norris Jokes Widget](http://wordpress.org/extend/plugins/chuck-norris-joke-widget/ "Chuck Norris Jokes Widget").

== Description ==

This plugin adds a small Wordpress widget that shows a random personalized Chuck Norris joke on your blog, starring yourself.
These jokes are taken from a database on icndb.com.

== Installation ==

To install this plugin:

1. Upload `personalized-chuck-norris-jokes.php` to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Drag the "Personalized Chuck Norris Joke" widget in a widget area through the 'Appearance - Widgets' menu.
1. Adjust the name of the main character in the joke. Then press save.

You should now see a different Chuck Norris joke on your blog every time the page loads.

== Frequently Asked Questions ==

Have a question? Be sure to ask it at [the plugin homepage](http://maartendecat.be/personalized-chuck-norris-joke-wordpress-plugin/ "Personalized Chuck Norris Jokes Wordpress Plugin Homepage")

== Screenshots ==

1. A Chuck Norris joke.

== Changelog ==

= 0.5 =

* Branched from "Chuck Norris Joke Widget", personalization will only be present in this plugin.

== Upgrade Notice ==

= 0.5 =

Branched from "Chuck Norris Joke Widget", personalization will only be present in this plugin.
